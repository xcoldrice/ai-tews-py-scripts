...
def get_affected_areas(polygons):
    ...
    for index, shape in municipality_shapes.iterrows():
        ...
        if not str(shape.geometry) == 'None' and 'lake' not in municipality.lower():
            for index in range(len(polygons)):
                try:
                    polygon = geometry.Polygon(polygons[index])
            ...
            if area_ctr > 0:
                ...
                if coverage > 0:
                    affected_areas.append({'coverage': coverage, 'municipality':municipality, 'province': province, 'barangay':barangay})
...
def generate(data, threshold, radar_name, dt):
    ...
    values = (radar_name, 'lightning', 'barangay', str(threshold), dt, str(json.dumps(aa)), str(json.dumps(lp)), str(datetime.now()), str(datetime.now()))
...
municipality_shapes = geopandas.read_file('shapes/Barangays/Barangays.shp')
radar_name = 'Subic'
